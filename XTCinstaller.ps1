...
try {
	...
	# 根据输入处理 APK 文件
	if ($inputMsg -match "^(http://|https://|ftp://)") {
		Write-Host "正在下载软件..."
		# 使用 curl 下载文件到设备
		& $adbPath shell "curl --output /sdcard/apk.apk $inputMsg" | ForEach-Object { Write-Host $_ }
	} else {
		# 检查本地文件是否存在
	...
	Write-Host "操作失败，错误信息:`n${exceptionName}: ${exceptionInfo}"
}
finally {
